...
<center>
	<div class="head">
	<h2> ( Profit / Loss ) REPORT CITY WISE</h2>
	</div>
...
<div style="margin-left:30%; margin-top:3%;">	
	...
	include "odsms_config.php";
		...
		//aaquddos $stid = $row11;
			...
			$res=mysqli_query($conn,"SELECT P_AMT('$start','$end','$stid') AS PAMT;") or die(mysqli_error($conn));
	...
	<?php
	$sql = "SELECT pid,storeid,medid,userid,pqty,pprice,pcost,pdate,pexp FROM purchase 
			WHERE pdate >= '$start' AND pdate <= '$end' AND storeid in (SELECT storeid from store where city='$city')";
	...
	$sql = "SELECT saleid,storeid,s_date,s_time,total_amt FROM sales
			WHERE s_date >= '$start' AND s_date <= '$end' AND storeid in (SELECT storeid from store where city='$city')";
